...
const VerticalNavLink = ({
  ...
  const isNavLinkActive = () => {
    const cleanBaseRoutePath = router.pathname?.split('/')[1]
    const path = `/${cleanBaseRoutePath}`
    if (router.pathname === item.path || handleURLQueries(router, item.path) || item.path === path) {
      return true
    }
